...
// WriteColoBuckets 把 Top 结果按 colo 分桶输出。
// 输出到 {dir}/colo/{COLO}.csv，每桶内按速度降序。
// 每桶只保留前 TopPerColo 个（默认 10）。
func WriteColoBuckets(dir string, results []DownloadResult) error {
	coloDir := filepath.Join(dir, "colo")
	if err := os.MkdirAll(coloDir, 0o755); err != nil {
		return err
	}

	// 按 colo 分桶
	buckets := make(map[string][]DownloadResult)
	for _, r := range results {
		colo := r.Colo
		if colo == "" {
			colo = "UNKNOWN"
		}
		buckets[colo] = append(buckets[colo], r)
	}

	// 每个 colo 内按速度降序，写出
	for colo, rs := range buckets {
		// 简单排序：速度降序（结果一般已经排好，但确保）
		for i := 0; i < len(rs); i++ {
			for j := i + 1; j < len(rs); j++ {
				if rs[j].Speed > rs[i].Speed {
					rs[i], rs[j] = rs[j], rs[i]
				}
			}
		}

		path := filepath.Join(coloDir, colo+".csv")
		f, err := os.Create(path)
		if err != nil {
			return err
		}
		...
		w := csv.NewWriter(f)
		w.Write([]string{"ip", "download_speed_MBps", "colo", "tier"})
		for _, r := range rs {
			w.Write([]string{
				r.IP.String(),
				fmt.Sprintf("%.4f", r.Speed),
				r.Colo,
				fmt.Sprintf("%d", r.Tier),
			})
		}
		w.Flush()
	}
